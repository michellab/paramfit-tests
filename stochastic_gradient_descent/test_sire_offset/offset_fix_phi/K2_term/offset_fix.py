...
def evaluate_phipsi(solute,phi,psi):
    ...
    phi_val = (float(space.calcDihedral(at0coords,at1coords,at2coords,at3coords).toString().split()[0]))
    ...
    psi_val = (float(space.calcDihedral(at0coords,at1coords,at2coords,at3coords).toString().split()[0]))
...
def dihedrals_offset(dict_structures):
    ...
    psi_3cos = 0.0
    counter= 0.0
    #do we need to sum multiple times
    # the same contribution?
    #let's suppose we have 10 times the same value of phi
    check_phi = []
    #sum over all unique phi values
...
#        print(dict_structures[key][2])
        if phi_angle in check_phi:
            #print("no")
            continue
        else:
...
def differences(dict_structures,offset,namefile):
    ...
    for key in dict_structures :
        ...
        emm_trunc_minus =emm_offset + (  (0.27)*(math.cos(2.*phi_angle))  - (0.27)*phi_2cos )
...
mdcrd_files = glob.glob("test/*/*.mdcrd")
counter_files = 0
good_structures = 0
...
#out_f = open("compare_energies.dat","w")
...
phi1,phi2,phi3 = dihedrals_offset(dict_structures)
