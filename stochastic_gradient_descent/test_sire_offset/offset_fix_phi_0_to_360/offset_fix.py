...
def differences(dict_structures,offset,namefile):
    ...
    for key in dict_structures :
        ...
        deg = dict_structures[key][2]  #phi rad


        #now compute also teh eqm -  right dihedral form

        emm_offset = emm + offset
        emm_trunc_minus =emm_offset + ( + 0.27*(math.cos(2.*phi_angle)) -0.27*phi_2cos +  0.45*(math.cos(3.*phi_angle))  -0.45*phi_3cos)

        off_phi = 0.27*phi_2cos + 0.45*phi_3cos
        diff = eqm - emm_trunc_minus
        print("EQM: %.4f EMM: %.4f Off:  %.4f  Phi_OFF: %.4f\n " %(eqm,emm,offset,off_phi))
        print("Phi: %.4f Deg: %.4f Diff: %.4f\n" % (phi_angle,deg,diff))
        #print(diff)
        #print(new_diff)
