...
def dihedrals_offset(dict_structures):
    ...
    return psi_1cos,psi_2cos,psi_3cos
...
def differences(dict_structures,offset,namefile):
    ...
    psi_1cos,psi_2cos,psi_3cos = dihedrals_offset(dict_structures)
    ...
    for key in dict_structures :
        ...
        psi_angle = dict_structures[key][3]*(math.pi/180.0)  #phi rad
        ...
        print(diff)
        new_rel_en.append(emm_trunc_minus)

        #all right now write the energies
        output_en.write("%.9f\t\t%.9f\t\t%.9f\n" %\
...
mdcrd_files = glob.glob("PhiPsi_1/5/*.mdcrd")
counter_files = 0
good_structures = 0
