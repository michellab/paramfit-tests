...
def nplusone_offset(quantum,amber):
...
def differences(dict_structures,offset,namefile):
    ...
    output_en.write("ff99SB\t\tff99SBnodih   mm0\t\tmm0+offset\t\tmm0+dihedral\t\tmm0+truncated\t\tmm0+trunc-\t\tmm0+trunc+\n")
    ...
    for key in dict_structures :
        ...
        psi_angle = dict_structures[key][3]*(math.pi/180.0)  #psi rad
        ...
        emm_trunc_minus =emm_offset + (  0.27*(math.cos(2*phi_angle)) - 0.27*phi_2cos +  0.42*(math.cos(3*phi_angle))  -0.42*phi_3cos +\
                                      0.45*(math.cos(psi_angle  - math.pi)) -0.45*psi_1cos    +  1.58*(math.cos(2*psi_angle  - math.pi)) - 1.58*psi_2cos +\
                                      0.55*(math.cos(3*psi_angle - math.pi)) -0.55*psi_3cos       )




        #all right now write the energies
        output_en.write("%.3f\t\t%.3f\n" %\
                        (eqm,emm_trunc_minus)   )
...
def differences_min(dict_structures,offset,namefile):
    ...
    output_en = open(namefile,"w")
    output_en.write("ff99SB\t\tmm0+trunc-\tdiff\n")
    ...
    for key in dict_structures :
        ...
        if eqm == emm :
            ...
            #all right now write the energies
            output_en.write("%.3f\t\t%.3f\t\t%.3f\n" %\
                        (eqm,emm_trunc_minus,diff)   )
...
#2) 1/N(N-1) offset
#3) LLS K offset
#4) Offset normal  :if en_i = en_j = 0.00 offset = 0.00 and we reweight the offset on the other structures

#1)normal offset as differences averages
offset = normal_offset(rel_orig,rel_zero)
print("Normal offset")
print(offset)
#2) N+1 offset
#off_nplusone = nplusone_offset(rel_orig,rel_zero)
#print("N+1 offset")
#print(off_nplusone)
#3) LLS offset
...
#print("lls offset")
#lls_offset = offset_compute_sgd(dict_structures)
#4)minima offset
print("minima offset")
min_offset = exclusion_offset(rel_orig,rel_zero)
print(min_offset)
...
#differences(dict_structures,off_nplusone,"2_nplusone.dat")
#differences(dict_structures,lls_offset,"3_llsoff.dat")
differences_min(dict_structures,min_offset,"4_minoff.dat")
