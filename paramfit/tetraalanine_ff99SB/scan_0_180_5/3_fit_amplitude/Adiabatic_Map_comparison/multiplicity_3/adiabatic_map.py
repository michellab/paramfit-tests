...
#        print(f.split("/"))
        phi =int( f.split("/")[-2]) # to be more consistent, we know that in -2 there's phi
        psi =int( f.split("/")[-1].split(".mdcrd")[0].split("structure_")[1])
        #first fix phi and psi values:
        if phi > 180.0:
            phi = phi - 360.0
        if psi > 180.0 :
...
amber_coords = "../../../1_inputfiles/energies/PhiPsi_1/*/structure*.mdcrd"
...
dict_diff_ff99 = differences(dict_amber,dict_ff99SB)
#Create the plots
