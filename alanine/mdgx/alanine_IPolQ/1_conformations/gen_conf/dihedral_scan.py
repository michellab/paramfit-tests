...
def move_dihedral(system,phi,psi,fold_numb,top_file,step,phi_atoms,psi_atoms):
    ...
    for val_phi in range(0,360,step):
        ...
        for val_psi in range(0,360,step):
            ...
            psi_dir = "PhiPsi_" + str(fold_numb) +"/" + str(phi_new) + "/" + str(val_psi)
            if not os.path.exists(psi_dir):
                os.makedirs(psi_dir)
            #pass val_psi which is theactual value of psi for structure_Psi name
            crd_file = generateCoordFiles(solute.property("coordinates"),psi_dir,val_psi,natoms)
            #minimise the structure and recover the new coordinates
            crd_file = minimiseSander(crd_file,top_file,val_phi,val_psi,phi_atoms,psi_atoms,psi_dir,current_dir)
            #phi_now,psi_now = evaluate_phipsi(solute,dihedral_phi,dihedral_psi)
...
def minimiseSander(coord_file,top_file,val_phi,val_psi,phi_atoms,psi_atoms,folder,current_dir):
#here you need
#top
#crd
#phi atoms
...
#            min_crd_file = minimiseSander(crd_file,top_file,val_phi,val_psi,phi_atoms,psi_atoms)
    ...
    #Remember ntxo to save in rst7 rather than netcdf as normal now for amber16
...
DISANG=structure_0.fs""")
    ...
    os.system(cmd)
    cmd = "rm mdinfo min.out structure_%d.mdcrd structure_%d.f structure_%d.in" %(val_psi,val_psi,val_psi)
    os.system(cmd)
    os.chdir(current_dir)
    crd_file = "Conf%d.crd" %val_psi
...
def getReady_gcrt(gcrt_name,fold_numb,phi_atoms,psi_atoms,step):
    ...
    #line Freq keyword
    new_gcrt.write("# HF/6-31G* Opt=(ModRedundant) geom=nocrowd\n")
    new_gcrt.write("\n")
...
if __name__ == "__main__":
     ...
     try:
         ...
         top_file = os.path.abspath(top_file) #take the absolut epath of top
         crd_file = sys.argv[2]  #coordiantes
         crd_file = os.path.abspath(crd_file)
